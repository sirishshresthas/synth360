...
from src.core.LoadBlob import load_data
...
class DataProcessor(object):
    ...
    def pivot_rater_data(self): 
        ...
        df_self = self._remove_duplicates(df=df_self, id_key=self.id_key items_only=True)
        df_others = df_self = self.df[~self.df[self.rater_col_name].isin(['self', 'Self'])]
    ...
    def filter_data_with_all_raters(self, remove_other_raters: bool = True, remove_rater_list: List = ['Superior', 'Other']) -> pd.DataFrame:
        ...
        unique_rater_counts = self.df.groupby(self.id_key)[self.rater_col_name].nunique()
        ...
        if remove_other_raters: 
            self.df = self.df[~self.df[self.rater_col_name].isin(remove_rater_list)]
    ...
    def median_rater_counts(self, id_key: str = "ESI_Key", rater_col_name: str = "RaterType"):
        ...
        filtered_df = self.df[~self.df[self.rater_col_name].isin(['self', 'Self'])]
